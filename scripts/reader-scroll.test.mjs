...
test('Resume stores a page-relative scroll position that survives a viewport resize', () => {
  const value = { page: 70, mode: 'scroll', zoom: 1, pageOffset: 0.4 };
  const place = normalizePlace(value, 112);
  assert.equal(place.mode, 'scroll');
  assert.equal(place.pageOffset, 0.4);
  for (const width of [366, 900]) {
    const { pages } = scrollLayout(112, width, []);
    const top = scrollDestination(pages, place.page, place.pageOffset);
    assert.equal(pageAtOffset(pages, top), 69);
    assert.ok(Math.abs((top - pages[69].top) / pages[69].height - 0.4) < 1e-10);
  }
  assert.equal(
    normalizePlace({ ...value, pageOffset: Infinity }, 112).pageOffset,
    0,
  );
  assert.equal(normalizePlace({ ...value, pageOffset: 20 }, 112).pageOffset, 1);
});

test('Mobile page gaps are compact without changing story jump destinations', () => {
  const { pages, height } = scrollLayout(112, 390, [], 12);
  for (let i = 1; i < pages.length; i++) {
    assert.equal(pages[i].top - pages[i - 1].top - pages[i - 1].height, 12);
    assert.equal(pageAtOffset(pages, scrollDestination(pages, i + 1)), i);
  }
  assert.equal(height, pages.at(-1).top + pages.at(-1).height + 12);
});
